...
export default class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.steps.push(
      this.add
        .image(0, 0, TextureKeys.Footprints, Phaser.Math.Between(0, 3))
        .setAlpha(1)
        .setDepth(DepthKeys.FOOTPRINTS),
    );
    ...
    votingZonesLayer.objects.forEach((tiledObject) => {
      ...
      const rectangle = this.add
        .rectangle(x!, y!, width, height, 0x9966ff, 0.5)
        .setStrokeStyle(4, 0xefc53f)
        .setOrigin(0)
        .setDepth(DepthKeys.VOTING_ZONE);
      this.add
        .bitmapText(
          x! + width! / 2,
          y! + height! / 2,
    ...
    this.physics.add.overlap(
      [this.currentPlayerThrowableWeapons, this.restOfPlayersThrowableWeapons],
      this.players,
    ...
    this.socket.on(
      ...
      (playersInitialStates: PlayersInitialStates) => {
        ...
        Object.keys(playersInitialStates).forEach((id) => {
          const isMainPlayer =
            playersInitialStates[id].playerId === this.currentPlayerId;
    ...
    this.socket.on(
      NetworkEventKeys.PlayersStatusUpdate,
      ...
      if (gameState.currentPlayer !== undefined) {
        if (this.preventDuplicateInitialSettingsEvent) {
          ...
          this.players
            .getChildren()
            .map((gameObject) =>
              this.handlePlayerVisibilityWhileVoting(gameObject as Player),
            );
        }
      }
    });
    ...
    autorun(() => {
      ...
      (this.players.getChildren() as Player[]).forEach(
        (player) => (player.enableBlood = enableBlood),
      );
    });
    ...
    this.socket.on(
      ...
      (playerSettings: PlayerSettings) => {
        if (playerSettings.id) {
          ...
          playerSettings.isVoter
            ? gameVotingManager.addPlayer(playerId)
            : gameVotingManager.removePlayer(playerId);
        } else {
          console.error(
            "Can't update remote player settings without player's id: ",
            playerSettings,
          );
        }
      },
    ...
    this.socket.on(
      NetworkEventKeys.PlayerFightAction,
      (action: PlayerFightAction) => {
  ...
  createVotingZoneEmitter(): void {
    if (!this.votingZoneParticlesEmitter) {
      // Emitter when the voting zone is active
      this.votingZoneParticlesEmitter = this.add
        .particles(TextureKeys.Stars)
        .setDepth(DepthKeys.VOTING_ZONE + 1)
        .createEmitter({
    ...
    if (!this.votingZoneExplodeParticlesEmitter) {
      // Emitter when the voting zone turns to inactive
      this.votingZoneExplodeParticlesEmitter = this.add
        .particles(TextureKeys.Stars)
        .setDepth(DepthKeys.VOTING_ZONE + 1)
        .createEmitter({
          frame: { frames: [0, 1, 2, 3], cycle: false },
        ...
        .onParticleDeath(() => {
          if (
            this.votingZoneExplodeParticlesEmitter?.getAliveParticleCount() ===
            0
          ) {
            // Stop itself
  ...
  updateVotingZoneEmitter(
    x: number,
    y: number,
    votingZone: Phaser.Types.GameObjects.Particles.RandomZoneSource,
  ): void {
    if (
      !this.votingZoneParticlesEmitter ||
      !this.votingZoneExplodeParticlesEmitter
    ) {
  ...
  private createRainParticles(): void {
    this.rainParticlesEmitter = this.add
      .particles(TextureKeys.Rain)
      .setDepth(DepthKeys.RAIN)
      .createEmitter({
        frame: 0,
        x: {
          min: this.cameras.main.worldView.left - 150,
          max: this.cameras.main.worldView.right + 150,
        },
        y: {
          min: this.cameras.main.worldView.top - 200,
          max: this.cameras.main.worldView.top,
        },
        lifespan: 600,
  ...
  setPlayersHealthBarVisibility(visibility: boolean): void {
    (this.players.getChildren() as Player[]).forEach(
      (player) => (player.healthBarIsVisible = visibility),
    );
  }
  ...
  updatePlayerState(newPlayerState: PlayerState): void {
    ...
    player.setPosition(
      newPlayerState.x + errorOffset,
      newPlayerState.y + errorOffset,
    );
  ...
  private handleFootprintTrail(player: Player): void {
    if (player.id === this.currentPlayerId && this.steps.length > 0) {
      ...
      const doStep: boolean =
        sandTile &&
        sandTile.properties?.groundType === 'sand' &&
        !vegetationTile;
      if (
        doStep &&
        Math.hypot(this.steps[0].x - player.x, this.steps[0].y - stepY) > 50
      ) {
  ...
  private handleFightEffects(): void {
    ...
    if (
      this.currentPlayerVision &&
      this.currentPlayerVision.active &&
      followedPlayer
    ) {
      this.currentPlayerVision.setPosition(followedPlayer.x, followedPlayer.y);
    }
    this.rainParticlesEmitter?.setPosition(
      {
        min: this.cameras.main.worldView.left - 150,
        max: this.cameras.main.worldView.right + 150,
      },
  ...
  private handleFootprintTrailVanish(delta: number): void {
    this.steps.forEach((step: Phaser.GameObjects.Image) =>
      step.setAlpha(step.alpha - delta / 4000),
    );
  }
  ...
  updateVotingZoneRender(newVotingZoneValue: VotingZoneValue) {
    const getVotingZone = (value: VotingZoneValue) =>
      this.votingZones.find((votingZone) => votingZone.value === value);
    if (this.currentVotingZoneValue === undefined) {
      if (newVotingZoneValue) {
        ...
        if (votingZone?.zone) {
          this.updateVotingZoneEmitter(
            votingZone.zone.x,
            votingZone.zone.y,
            votingZone.zone.geom,
          );
  ...
  addPlayer(playerInitialState: PlayerInitialState, isMainPlayer = true): void {
    const player = PlayerFactory.fromPlayerInitialState(
      this,
      playerInitialState,
    );
    ...
    if (isMainPlayer) {
      ...
      this.renderTexture.setMask(
        new Phaser.Display.Masks.BitmapMask(this, this.visionMaskContainer),
      );
    ...
    gameState.addPlayer(
      player.id,
      playerInitialState.playerSettings,
      isMainPlayer,
    );
